...
workers_available_nif_test_() ->
    {setup, fun() -> ok = start_with(nif) end,
     [{timeout, 5000,
       fun() ->
               ?assert(bcrypt:workers_available()),
               {ok, BigSalt} = bcrypt:gen_salt(15),
               {ok, Salt} = bcrypt:gen_salt(),
               spawn(bcrypt, hashpw, ["foo", BigSalt]),
               bcrypt:hashpw("foo", Salt),
               ?assert(bcrypt:workers_available()),
               spawn(bcrypt, hashpw, ["foo", BigSalt]),
               bcrypt:hashpw("foo", Salt),
               ?assertNot(bcrypt:workers_available())
       end}]}.
